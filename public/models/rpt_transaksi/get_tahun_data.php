...
if($divisi == "All"){
  ...
  foreach($data_bln as $bln){
    ...
    if($bln == 1){
          $mont = 01;
          $get_data =get_data_divisi($conn,$tahun,$mont,$divisi);
          $rowdata[] = $get_data;
...
function get_data_divisi($connection,$tahun,$monts,$divisi){

  $bln_s = bulan($monts);
 $sql =" SELECT DISTINCT kategory,divisi FROM master_video WHERE  tahun='".$tahun."' AND bulan ='".$bln_s."' AND divisi ='".$divisi."'";
 $result = odbc_exec($connection,$sql);
 while($arr = odbc_fetch_array($result)){
       $datas[] = $arr;
 }; 
